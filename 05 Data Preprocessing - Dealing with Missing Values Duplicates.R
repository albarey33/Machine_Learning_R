...
required_packages <- c("rlang", "ggplot2", "dplyr", "repr", "data.table",
                       "hexbin", "GGally")
...
str(auto_prices)
dim(auto_prices)
table(auto_prices$symboling, useNA = 'always')
...
auto_prices[,'symboling'] <- NULL  # By some reason this column is eliminated in edx
...
dim(auto_prices)
col <- c("price", "bore", "stroke", "horsepower", "peak.rpm", "num.of.doors")
...
##################################################################.
# 5 REMOVE DUPLICATE RECORDS DATA GERMAN CREDIT ----- 
